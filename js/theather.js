$(function () {
    // 뷰포트의 가로길이가 1024px이상이면 pc버전,
    // 1024미만이면 모바일 버전으로 주메뉴 애니메이션 설정
    ...
    const proAni25 = gsap.timeline()
        .to(".product-box .cont .img-box3", {
            opacity: 1, delay: 10, duration: 10
        });
    ...
    // .custom-cursor-area영역에 마우스 무브 이벤트 설정
    ...
    // custom-cursor-area영역에서 마우스를 아웃하면 커스텀 커서 사라짐
    ...
    const storeSwiper = new Swiper(".storeSwiper", {
        ...
        loop: true,
        navigation: {
            nextEl: '.storeSwiper .swiper-button-next',
            prevEl: '.storeSwiper .swiper-button-prev'
        },
        breakpoints: {
            1200: {
                slidesPerView: 5,
    ...
    $(window).scroll(function () {
        // 화면 위쪽에서는 top-btn이 안보이다가 500px정도 스크롤 되면
        // top-btn 보임
        ...
        // 스크롤 내릴 때마다 section안의 자식 컨텐츠들의 애니메이션 실행
        // section개수만큼 반복
    ...
    // 변수 선언할때 $를 변수명에 붙이면 변수에 저장된 객체의 속성값도 저장
    function headerHide() {
        let $window = $(window);
        let $header = $('header');
        ...
        $window.on("scroll", function () {
            ...
            // 만약 window객체의 scrollTop값이 header의 높이값보다 커지면
            // header에 active클래스를 추가하고, header의 높이값보다 크지 않으면 header에서 active클래스를 제거
            ...
            function has_scroll() {
                ...
                // 만약 last_scroll_current_scroll값이 move_Scroll값보다 적게 스크롤되면 아무 반응 없음.
    ...
    // section의 data-bg속성값에 따라 baby의 배경색 변경하기
    // $section변수에 section객체를 모두 저장
    ...
    $(window).on('scroll', function () {
        ...
        const $section = $sections.map(function () {
